...
- **新增即梦账号池**：支持保存多个即梦账号 Cookie，支持账号单独启停、轮询调用、失败冷却和状态刷新；Cookie 使用 AES-256-GCM 加密保存。
- **新增账号状态查询**：增加 `GET /v1/account/status`，可查询当前账号会员状态、积分及免费积分余量，不暴露 Cookie。
- **修复视频参考图**：修复参考图上传成功但未参与视频生成的问题，正确使用图生视频模式；同时兼容聊天多模态图片和 multipart 多文件上传。
- **模型自动适配**：补充 Seedance 2.5 等新模型映射，优先使用即梦官网动态模型配置，并保留静态模型兜底。
- **管理端优化**：管理网页面完成中文化和界面美化，优化账号池、Key 管理及状态展示。
- **Docker 本地部署**：完成 Docker 镜像构建和本地容器部署，保留数据目录及账号池加密密钥。
